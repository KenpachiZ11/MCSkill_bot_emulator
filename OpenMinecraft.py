...
def emulate_lkm_press():
    """Эмулирует зажатую ЛКМ до закрытия лаунчера."""
    global launcher_closed
    try:
        print("Запускаю цикл эмуляции зажатой ЛКМ ...")
        pyautogui.mouseDown()  # Нажимаем и держим ЛКМ

        # Этот цикл будет выполняться, пока лаунчер не будет закрыт
        while not launcher_closed:
            time.sleep(0.1)  # Небольшая задержка, чтобы не перегружать процессор

        pyautogui.mouseUp()  # Отпускаем ЛКМ
        print("Цикл эмуляции зажатой ЛКМ завершен.")
